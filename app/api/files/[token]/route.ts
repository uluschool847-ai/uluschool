...
import { canAccessPrivateStorageKey } from "@/lib/repositories/file-access-repository";
...
function errorResponse(status: 400 | 401 | 404 | 503, error: string) {
...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    if (!storageKey.startsWith("private/")) return errorResponse(400, "Bad request");
  } catch {
