...
vi.mock("next/server", () => {
  return {
    NextResponse: {
      ...
      rewrite: (...args: unknown[]) => rewriteMock(...(args as [string | URL])),
      json: (...args: unknown[]) =>
        jsonMock(...(args as [unknown, { status: number } | undefined])),
    },
  };
});
...
describe("Middleware Routing and Access Control", () => {
  ...
  describe("Role-Based Access Control (RBAC) - Unauthenticated", () => {
    ...
    it("redirects Guest trying to access protected portal routes to login", async () => {
      const req = createMockRequest("/portal/teacher");
      await middleware(req);
  ...
  describe("Advanced Auth Routing & Preservation", () => {
    it("preserves destination via next parameter for unauthenticated requests", async () => {
      ...
      const redirectUrl = redirectMock.mock.lastCall?.[0]?.toString() || "";
      expect(redirectUrl).toContain("/portal/login");
      expect(redirectUrl).toContain("callbackUrl=%2Fportal%2Fteacher%2Fgrades");
    });
    ...
    it("rejects cross-role access with a hard 403 or explicit unauthorized state (not a soft fallback redirect)", async () => {
      ...
      const is403Response =
        jsonMock.mock.calls.length > 0 && jsonMock.mock.lastCall?.[1]?.status === 403;
  ...
  describe("Teacher Portal Route Behavior", () => {
    it("redirects guest /portal/teacher to login with callbackUrl", async () => {
      const req = createMockRequest("/portal/teacher");
      await middleware(req);
    ...
    it("allows an active teacher cookie to pass /portal/teacher as a first-level check", async () => {
      ...
      expect(verifySessionToken).toHaveBeenCalledWith("teacher-cookie");
      expect(redirectMock).not.toHaveBeenCalled();
      expect(res).toEqual(expect.objectContaining({ cookies: expect.any(Object) }));
    });
    ...
    it("redirects admin cookie on /portal/teacher to unauthorized or an admin route pattern", async () => {
      ...
      const redirectUrl = redirectMock.mock.lastCall?.[0]?.toString() || "";
      expect(redirectUrl.includes("/portal/unauthorized") || redirectUrl.includes("/admin")).toBe(
        true,
      );
    });
