...
export async function getParentDashboardData(parentId: string) {
  ...
  const childrenData = parent.children.map((child) => {
    const completedCount = child.submissions.length;
    const groupLessons = (child.enrolledClassGroups ?? []).flatMap((group) =>
      group.lessons.map((lesson) => ({
        ...lesson,
        classGroup: lesson.classGroup ?? { id: group.id, name: group.name },
      })),
    );
    const upcomingClasses = [...child.enrolledClasses, ...groupLessons]
      .sort((left, right) => left.startAt.getTime() - right.startAt.getTime())
      .slice(0, 3);
    ...
    return {
      ...
      upcomingClasses: upcomingClasses.map((scheduledClass) => ({
        id: scheduledClass.id,
        title: scheduledClass.title,
        time: scheduledClass.startAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
        teacher: scheduledClass.teacher?.fullName ?? "TBA",
        subject: scheduledClass.subject,
        classGroup: scheduledClass.classGroup,
      })),
      homeworkStatus: child.submissions.map((submission) => ({
        id: submission.id,
        title: submission.assignment.title,
        status: submission.grade ? "Graded" : "Submitted",
        dueDate: submission.assignment.dueDate.toLocaleDateString(),
      })),
      recentGrades: child.submissions
      ...
      structuredProgress: {
        attendance: "95%",
        completedAssignments: `${completedCount} tasks`,
      },
    };
  });

  return Object.assign(childrenData, { children: childrenData });
}
