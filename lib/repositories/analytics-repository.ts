...
export async function getAdminAnalyticsOverview() {
  const [totalApplications, acceptedApplications, totalContactLeads, enquiriesBySource, contactBySource] =
    await Promise.all([
  ...
  const conversionRate = totalApplications === 0 ? 0 : (acceptedApplications / totalApplications) * 100;
...
export async function getAdvancedBIMetrics() {
  const [totalPayments, allPayments, activeSubscriptions, cancelledSubscriptions] = await Promise.all([
  ...
  const ltv = totalStudentsWithPayments.length > 0 ? totalRevenue / totalStudentsWithPayments.length : 0;
  ...
  const revenueChartData = Array.from(monthlyRevenueMap.entries()).map(([month, amount]) => ({ month, amount }));
